...
def evaluate_model(model, X_test, y_test, model_name, run_id):
    predictions = model.predict(X_test)
    accuracy = accuracy_score(y_test, predictions)
    report = classification_report(y_test, predictions, output_dict=True)
    cm = confusion_matrix(y_test, predictions)
    
    # Log metrics and confusion matrix with MLflow
...
if __name__ == "__main__":
    ...
    from model_training_mlflow import train_xgboost, train_random_forest, train_knn
    ...
    # Évaluer les modèles
    xgboost_accuracy, xgboost_report, xgboost_cm = evaluate_model(xgboost_model, X_test, y_test, "XGBoost Model", xgboost_run_id)
    rf_accuracy, rf_report, rf_cm = evaluate_model(rf_model, X_test, y_test, "Random Forest Model", rf_run_id)
    knn_accuracy, knn_report, knn_cm = evaluate_model(knn_model, X_test, y_test, "KNN Model", knn_run_id)
